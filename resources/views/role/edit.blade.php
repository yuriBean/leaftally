...
<div class="modal-body p-6 bg-[#FAFBFC]">
  <div class="bg-white rounded-[8px] border border-[#E5E7EB] p-6 shadow-sm overflow-hidden">
    ...
    @php
      ...
      $modules=['dashboard','user','role','invoice','bill','revenue','payment','invoice product','bill product','goal','credit note','debit note','bank account','employee','set salary','pay slip','transfer','transaction','product & service','customer','vender','plan','contract','constant tax','constant category','constant unit','constant custom field','constant contract type','constant bank','branch','designation','department','loan option','allowance option','payslip type','document type','company settings','assets','chart of account','journal entry','report','bom','production','allowance',
            'commission',
            'loan',
          ...
          @if(!empty($permissions))
            ...
            <table class="table table-roles w-full text-[12px] border border-[#E5E5E5] rounded-[4px]" id="dataTable-1">
              ...
              @foreach($visibleModules as $module)
                @php $slug = str_replace(' ', '', $module); @endphp
                <tr class="border border-[#E5E5E5]">
                  <td class="border-r border-[#E5E5E5] py-2 px-4">
                      ...
                      @if(in_array('manage '.$module, $permNames))
                        ...
                        @if($key !== false)
                          <div class="col-md-4 custom-control custom-checkbox">
                            {{Form::checkbox('permissions[]',$key,$role->permission, ['class'=>'form-check-input isscheck isscheck_'.$slug,'id'=>'permission'.$key])}}
...
<div class="modal-footer">
  ...
  <input type="submit" value="{{__('Update')}}" class="btn  py-[6px] px-[10px] btn bg-[#007C38] text-white hover:bg-green-700">
</div>
{{Form::close()}}
