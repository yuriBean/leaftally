...
@section('breadcrumb')
  ...
  <li class="breadcrumb-item">{{ __('BOMs') }}</li>
@endsection
...
<div class="flex items-center gap-2 mt-2 sm:mt-0">
  {{-- Export (ALL) --}}
  <a class="flex items-center gap-2 border border-[#007C38] text-[#007C38] bg-white px-4 py-2 rounded-[6px] text-[14px] font-[500] hover:bg-[#007C3808] transition-all duration-200 shadow-sm min-w-fit"
     href="{{ route('bom.export') }}"
  ...
  @can('create bom')
    <a href="{{ route('bom.create') }}"
      ...
      <i class="ti ti-plus"></i> {{ __('Create BOM') }}
    </a>
  @endcan
</div>
@endsection
...
@section('content')
  ...
  {{-- Bulk actions toolbar (appears only when rows selected) --}}
  @can('delete bom')
  <div class="mt-4">
      <x-bulk-toolbar
      :deleteRoute="route('bom.bulk-destroy')"
      :exportRoute="route('bom.export-selected')"
      scope="boms"
      tableId="boms-table"
      selectedLabel="{{ __('BOM selected') }}"
    />
  </div>
  @endcan
    ...
    <div class="col-md-12 ">
      ...
      <div class="card-body bg-white m-4">
        <div class="table-responsive table-new-design bg-white">
          <table id="boms-table" class="table datatable border border-[#E5E5E5] rounded-[8px]">
            <thead class="bg-[#F6F6F6]">
              <tr>
                ...
                <th data-sortable="false" data-type="html"
                  ...
                  <label class="mcheck">
                    <input type="checkbox" class="jsb-master" data-scope="boms">
                    <span class="box"></span>
                  </label>
                </th>
                <th class="px-4 py-1 border border-[#E5E5E5] font-[600] text-[12px]">{{ __('Code') }}</th>
                <th class="px-4 py-1 border border-[#E5E5E5] font-[600] text-[12px]">{{ __('Name') }}</th>
                <th class="px-4 py-1 border border-[#E5E5E5] font-[600] text-[12px]">{{ __('Active') }}</th>
                <th class="px-4 py-1 border border-[#E5E5E5] font-[600] text-[12px] text-center">{{ __('Inputs') }}</th>
                <th class="px-4 py-1 border border-[#E5E5E5] font-[600] text-[12px] text-center">{{ __('Outputs') }}</th>
                @if (Gate::check('edit bom') || Gate::check('delete bom') || Gate::check('create production'))
            ...
            <tbody>
              @foreach ($boms as $bom)
                <tr class="bom_tr"
                    data-url="{{ route('bom.show', $bom->id) }}"
                    data-id="{{ $bom->id }}">
                  {{-- row checkbox --}}
                  ...
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                    <a href="{{ route('bom.show', $bom->id) }}"
                       class="border border-[#137051] leading-[24px] text-[#137051] rounded-[4px] text-[12px] font-[500] px-5">
                      {{ $bom->code }}
                    </a>
                  </td>
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $bom->name }}</td>
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                    @if($bom->is_active)
                  ...
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700 text-center">
                    {{ $bom->inputs_count ?? $bom->inputs()->count() }}
                  </td>
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700 text-center">
                    {{ $bom->outputs_count ?? $bom->outputs()->count() }}
                  </td>
                  ...
                  @if (Gate::check('edit bom') || Gate::check('delete bom') || Gate::check('create production'))
                    <td class="Action px-4 py-3 text-right relative border border-[#E5E5E5]">
                      ...
                      <div class="dropdown-menu dropdown-menu-end mt-0 w-[220px] bg-white border rounded-md shadow-lg text-sm p-0">
                        @can('create production')
                          <li>
                            <a href="{{ route('production.create') }}?bom_id={{ $bom->id }}"
                              ...
                              <i class="ti ti-player-play"></i><span>{{ __('Create Production') }}</span>
                            </a>
                          </li>
                        @endcan
                        @can('edit bom')
                        ...
                        @can('delete bom')
                          <li>
                            {!! Form::open(['method' => 'DELETE', 'route' => ['bom.destroy', $bom->id]]) !!}
                              <a href="#"
                                 class="dropdown-item flex items-center gap-2 w-full px-4 py-2 hover:bg-[#007C3812] bs-pass-para">
