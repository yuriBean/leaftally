...
@section('action-btn')
  @can('create allowance option')
    <div class="flex items-center gap-2 mt-2 sm:mt-0 float-end">
      <a href="#"
         data-url="{{ route('allowanceoption.create') }}"
      ...
      <div class="h-1 w-full" style="background:#007C38;"></div>
        ...
        <div class="table-responsive table-new-design bg-white p-4">
          <table class="table datatable border border-[#E5E5E5] rounded-[8px]">
            <thead>
              <tr>
                <th class="px-4 py-1 border border-[#E5E5E5] bg-[#F6F6F6] font-[600] text-[12px]">{{ __('Allowance Option') }}</th>
                      ...
                      @can('delete allowance option')
                        <li>
                          ...
                          <a href="#"
                             ...
                             data-confirm-yes="document.getElementById('delete-form-{{ $allowanceoption->id }}').submit();">
...
@push('script-page')
<script>
  // Prevent dropdown/menu clicks from bubbling into datatable (parity with Bills)
