...
@section('action-btn')
<style>
  /* material checkbox (same as Assets/Goals/Taxes) */
...
<div class="flex items-center gap-2 mt-2 sm:mt-0">
    {{-- Export All --}}
    <a href="{{ route('product-category.export') }}"
    ...
    @can('create constant category')
        <a href="#"
           data-url="{{ route('product-category.create') }}"
           data-ajax-popup="true" data-bs-toggle="tooltip"
           data-title="{{ __('Create New Category') }}" title="{{ __('Create') }}"
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            {{-- ✅ Bulk toolbar: delete + export selected (component already used in other modules) --}}
            @can('delete constant category')
                <x-bulk-toolbar
                    :deleteRoute="route('product-category.bulk-destroy')"
                    :exportRoute="route('product-category.export-selected')"
                    scope="categories"
                    tableId="categories-table"
                    selectedLabel="{{ __('Category(ies) selected') }}"
                />
            @endcan
            ...
            <div class="card-body table-border-style">
                <div class="table-responsive table-new-design bg-white p-4">
                    <table id="categories-table" class="table datatable border border-[#E5E5E5] rounded-[8px]">
                        <thead>
                            <tr>
                                {{-- master checkbox --}}
                                <th data-sortable="false" data-type="html"
                                    ...
                                    <label class="mcheck">
                                        <input type="checkbox" class="jsb-master" data-scope="categories">
                                        <span class="box"></span>
                                    </label>
                                </th>
                            ...
                            @foreach ($categories as $category)
                                <tr>
                                    ...
                                    <td class="input-checkbox px-4 py-4 text-left text-[12px] border-0 w-12">
                                        <label class="mcheck">
                                            <input type="checkbox" class="jsb-item" data-scope="categories" value="{{ $category->id }}" data-id="{{ $category->id }}">
                                            <span class="box"></span>
                                        </label>
                                    </td>

                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $category->name }}</td>
                                    ...
                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                                        {{ !empty($category->chartAccount) ? $category->chartAccount->name : '-' }}
                                    </td>
...
@push('script-page')
<script>
  // Prevent dropdowns/checkbox clicks from bubbling to row
