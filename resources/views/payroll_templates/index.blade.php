...
@push('script-page')
    <script type="text/javascript">
        $(document).on("click", ".payroll-template-checkbox", function() {
            var chbox = $(this);
            $.ajax({
...
@section('title')
    <div class="d-inline-block">
        @if (\Auth::user()->type == 'company')
            <h5 class="h4 d-inline-block font-weight-400 mb-0">{{ __('Payroll Email Notification') }}</h5>
        @else
            <h5 class="h4 d-inline-block font-weight-400 mb-0">{{ __('Payroll Email Templates') }}</h5>
        @endif
    </div>
@endsection
@section('breadcrumb')
    ...
    @if (\Auth::user()->type == 'company')
        <li class="breadcrumb-item active" aria-current="page">{{ __('Payroll Email Notification') }}</li>
    @else
        <li class="breadcrumb-item active" aria-current="page">{{ __('Payroll Email Template') }}</li>
    @endif
@endsection
@section('action-btn')
@endsection
                        ...
                        <table class="table datatable" id="pc-dt-simple">
                            ...
                            <tbody>
                                @foreach ($PayrollTemplates as $PayrollTemplate)
                                    <tr>
                                        <td class="px-4 py-3 border border-[#E5E5E5] text-[#323232] text-start">{{ $PayrollTemplate->name }}</td>
                                                ...
                                                @if (\Auth::user()->type == 'super admin')
                                                    <div class="text-start">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end mt-0 w-[190px] bg-white border rounded-md shadow-lg text-sm p-0">
                                                            <a href="{{ route('manage.payroll.language', [$PayrollTemplate->id, \Auth::user()->lang]) }}"
                                                ...
                                                @if (\Auth::user()->type == 'company')
                                                    <div class="text-start d-flex align-items-center justify-content-start gap-2">
                                                        ...
                                                        <div class="form-check form-switch d-inline-block">
                                                            <input class="form-check-input payroll-template-checkbox" type="checkbox"
                                                                id="payroll_template_{{ $PayrollTemplate->template->id }}"
                                                                @if ($PayrollTemplate->template->is_active == 1) checked="checked" @endif
                                                                value="{{ $PayrollTemplate->template->is_active }}"
                                                                data-url="{{ route('status.payroll.language', [$PayrollTemplate->template->id]) }}"
                                                                role="switch">
                                                            <label class="custom-control-label form-control-label"
                                                                for="payroll_template_{{ $PayrollTemplate->template->id }}"></label>
                                                        </div>
                                                    </div>
                                                @endif
                                            </div>
