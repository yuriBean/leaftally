...
@section('action-btn')
<style>
  /* material checkbox (shared across modules) */
...
<div class="flex items-center gap-2 mt-2 sm:mt-0">
    {{-- Export All --}}
    <a href="{{ route('product-unit.export') }}"
    ...
    @can('create constant unit')
        <a href="#"
           data-url="{{ route('product-unit.create') }}"
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            @can('delete constant unit')
                <x-bulk-toolbar
                    :deleteRoute="route('product-unit.bulk-destroy')"
                    :exportRoute="route('product-unit.export-selected')"
                    scope="units"
                    tableId="units-table"
                    selectedLabel="{{ __('Unit(s) selected') }}"
                />
            @endcan
            ...
            <div class="card-body table-border-style">
                <div class="table-responsive table-new-design bg-white p-4">
                    <table id="units-table" class="table datatable">
                        <thead>
                            <tr>
                                {{-- master checkbox --}}
                                <th data-sortable="false" data-type="html"
                                    ...
                                    <label class="mcheck">
                                        <input type="checkbox" class="jsb-master" data-scope="units">
                                        <span class="box"></span>
                                    </label>
                                </th>

                                <th class="px-4 py-1 border border-[#E5E5E5] bg-[#F6F6F6] font-[600] text-[12px]">{{ __('Unit') }}</th>
                        ...
                        @foreach ($units as $unit)
                            <tr>
                                ...
                                <td class="input-checkbox px-4 py-4 text-left text-[12px] border-0 w-12">
                                    <label class="mcheck">
                                        <input type="checkbox" class="jsb-item" data-scope="units" value="{{ $unit->id }}" data-id="{{ $unit->id }}">
                                        <span class="box"></span>
                                    </label>
                                </td>

                                <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $unit->name }}</td>

                                <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700 Action relative">
                                    <button
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end mt-0 w-[190px] bg-white border rounded-md shadow-lg text-sm p-0">
                                        @can('edit constant unit')
                                            <a href="#"
                                               ...
                                               data-title="{{ __('Edit Product Unit') }}"
                                        ...
                                        @can('delete constant unit')
                                            ...
                                            <a href="#"
                                               class="dropdown-item bs-pass-para flex text-[#323232] gap-2 w-full px-4 py-2 text-left hover:bg-[#007C3812]"
                                               data-title="{{ __('Delete Product Unit') }}"
...
@push('script-page')
<script>
  // keep row click from firing on checkbox/dropdown interactions
