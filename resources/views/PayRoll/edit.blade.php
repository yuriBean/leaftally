...
@php
  $monthes = collect(range(1, 12))->mapWithKeys(function ($month) {
      $value = now()->year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT) . '-01';
      $label = \Carbon\Carbon::parse($value)->format('F Y');
      return [$value => $label];
  });
@endphp

{{ Form::model($payroll, ['route' => ['payroll.update', $payroll->id], 'method' => 'PUT', 'class' => 'needs-validation', 'novalidate']) }}
            ...
            <div class="space-y-2">
              <div class="form-group mb-0">
                ...
                <div class="form-icon-user">
                  {{ Form::text('employee_id', optional($employees->firstWhere('id', $payroll->employee_id))->name, ['class' => 'form-control', 'disabled']) }}
                </div>
              </div>
            </div>
            ...
            @if ($payroll->payroll_month)
              <div class="space-y-2">
                <div class="form-group mb-0">
                  ...
                  {!! Form::select('payroll_month', $monthes, $payroll->payroll_month, [
                      'class' => 'form-control',
                      'required' => 'required',
                  ]) !!}
                </div>
              ...
              <div class="form-group mb-0">
                ...
                <div class="form-icon-user">
                  {{ Form::number('basic_salary', null, [
                    'class' => 'form-control',
                    (url()->previous() == url('/payroll')) ? '' : 'disabled',
                    'min'=>0, 'step'=>'0.01'
                  ]) }}
                </div>
              </div>
      ...
      <div class="bg-white rounded-[8px] border border-[#E5E7EB] mb-6 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
          <div id="allowances-wrapper" class="row">
            @php $allowances = $payroll->allowances ?? collect(); @endphp
            ...
            @else
              @foreach ($allowances as $index => $allowance)
                <div class="allowance-group grid grid-cols-1 md:grid-cols-2 gap-6 mb-2">
                  <div class="space-y-2">
                    {{ Form::label("allowances[{$index}][type]", __('Allowance Type'), ['class' => 'form-label']) }}
                    {!! Form::select("allowances[{$index}][type]", $allowanceTypes ?? [], $allowance->type, [
                        'class' => 'form-control',
                        'placeholder' => __('Select type')
                    ]) !!}
                  </div>
                  <div class="space-y-2">
                    ...
                    <div class="d-flex align-items-end gap-2">
                      <input type="number" name="allowances[{{ $index }}][amount]" value="{{ $allowance->amount }}" class="form-control" min="0" step="0.01">
                      @if ($index > 0)
                        <button type="button" class="btn btn-light btn-sm p-1" title="{{ __('Remove') }}" onclick="removeGroup(this,'allowances-wrapper','allowance-group','allowances')">
          ...
          <button type="button" onclick="addAllowanceBtn()" class="btn btn-outline-success mt-2 col-12">
      ...
      <div class="bg-white rounded-[8px] border border-[#E5E7EB] mb-6 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
          <div id="deductions-wrapper" class="row">
            @php $deductions = $payroll->deductions ?? collect(); @endphp
            ...
            @else
              @foreach ($deductions as $index => $deduction)
                <div class="deduction-group grid grid-cols-1 md:grid-cols-2 gap-6 mb-2">
                  <div class="space-y-2">
                    {{ Form::label("deductions[{$index}][type]", __('Deduction Type'), ['class' => 'form-label']) }}
                    {!! Form::select("deductions[{$index}][type]", $deductionTypes ?? [], $deduction->type, [
                        'class' => 'form-control',
                        'placeholder' => __('Select type')
                    ]) !!}
                  </div>
                  <div class="space-y-2">
                    ...
                    <div class="d-flex align-items-end gap-2">
                      <input type="number" name="deductions[{{ $index }}][amount]" value="{{ $deduction->amount }}" class="form-control" min="0" step="0.01">
                      @if ($index > 0)
                        <button type="button" class="btn btn-light btn-sm p-1" title="{{ __('Remove') }}" onclick="removeGroup(this,'deductions-wrapper','deduction-group','deductions')">
                          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <polyline points="3 6 5 6 21 6"></polyline>
                            <path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"></path>
                            <path d="M10 11v6M14 11v6"></path>
          ...
          <button type="button" onclick="addDeductionBtn()" class="btn btn-outline-danger mt-2 col-12">
      ...
      <div class="bg-white rounded-[8px] border border-[#E5E7EB] mb-6 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
          <div id="bonuses-wrapper" class="row">
            @php $bonuses = $payroll->bonuses ?? collect(); @endphp
            @if ($bonuses->isEmpty())
              ...
              @foreach ($bonuses as $index => $bonus)
                <div class="bonus-group grid grid-cols-1 md:grid-cols-2 gap-6 mb-2">
                  <div class="space-y-2">
                    ...
                    {!! Form::select("bonuses[{$index}][type]", $bonusTypes ?? [], $bonus->type, [
                        'class' => 'form-control',
                        'placeholder' => __('Select type')
                    ]) !!}
                  </div>
                  <div class="space-y-2">
                    ...
                    <div class="d-flex align-items-end gap-2">
                      <input type="number" name="bonuses[{{ $index }}][amount]" value="{{ $bonus->amount }}" class="form-control" min="0" step="0.01">
                      @if ($index > 0)
                        <button type="button" class="btn btn-light btn-sm p-1" title="{{ __('Remove') }}" onclick="removeGroup(this,'bonuses-wrapper','bonus-group','bonuses')">
                          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <polyline points="3 6 5 6 21 6"></polyline>
                            <path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"></path>
                            <path d="M10 11v6M14 11v6"></path>
          ...
          <button type="button" onclick="addBonusBtn()" class="btn btn-outline-primary mt-2 col-12">
...
<div class="modal-footer border-t border-[#E5E5E5] bg-[#FAFAFA] px-6 py-4 flex justify-end gap-3">
  ...
  <input type="submit" value="{{ __('Update') }}" class="btn py-[6px] px-[10px] bg-[#007C38] text-white hover:bg-[#005f2a]">
</div>
{{ Form::close() }}
...
<script>
  ...
  function buildOptions(map) {
    ...
    Object.entries(map).forEach(([value, label]) => { html += `<option value="${esc(value)}">${esc(label)}</option>`; });
    return html;
  }
