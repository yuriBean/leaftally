...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Pay Slip') }}</li>
@endsection

@section('action-btn')
    <div class="flex items-center gap-2 mt-2 sm:mt-0">
            <a href="{{ route('payrollSlip.export') }}" data-bs-toggle="tooltip" title="{{ __('Export') }}"
        ...
        <form action="{{ route('payroll.payAll') }}" method="POST">
            @csrf
            @method('PUT')
            <button type="submit" class="flex items-center gap-2 bg-[#007C38] text-white px-4 py-2 rounded-[6px] text-[14px] font-[500] hover:bg-[#005f2a] transition-all duration-200 shadow-sm min-w-fit">
                 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tbody>
                            @foreach ($payrolls as $k => $payroll)
                                <tr class="cust_tr" id="cust_detail"
                                    ...
                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                                        {{ $payroll['basic_salary'] }}</td>

                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                                        {{ $payroll['net_salary'] }}</td>

                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                                        {{ \Carbon\Carbon::parse($payroll['payroll_month'])->format('F') }}</td>

                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">
                                        @if (!empty($payroll['payment_date']))
                                            {{ \Carbon\Carbon::parse($payroll['payment_date'])->format('F') }}
                                        @else
                                            ----
                                        @endif
                                    </td>
                                        ...
                                        @if ($payroll['status'] === 'Paid')
                                            <span class="p-1 rounded-[4px] bg-[#137051] text-white"
                                                disabled>{{ __('Paid') }}</span>
                                        @else
                                        
                                        <button class="p-1 rounded-[4px] bg-[#BE123C] text-white"
                                        disabled>{{ __('Unpaid') }}</button>
                                        @endif
                                    </td>
                                    ...
                                    <td class="Action px-4 py-3 text-right relative border border-[#E5E5E5]">
                                        @if ($payroll['status'] === 'Paid')
                                            <button class="btn btn-sm btn-outline-secondary"
                                                disabled>{{ __('Paid') }}</button>
                                        @else
                                            <form action="{{ route('payroll.markPaid', $payroll->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('PUT')
