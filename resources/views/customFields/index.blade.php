...
@section('action-btn')
<style>
  /* material checkbox (same look used elsewhere) */
...
<div class="flex items-center gap-2 mt-2 sm:mt-0">
  {{-- Export All --}}
  <a href="{{ route('custom-field.export') }}"
  ...
  @can('create constant custom field')
    <a href="#"
       data-url="{{ route('custom-field.create') }}"
       data-bs-toggle="tooltip" title="{{ __('Create') }}"
       data-ajax-popup="true" data-title="{{ __('Create New Custom Field') }}"
  ...
  <div class="row">
    <div class="col-sm-12">
      ...
      @can('delete constant custom field')
        <x-bulk-toolbar
          :deleteRoute="route('custom-field.bulk-destroy')"
          :exportRoute="route('custom-field.export-selected')"
          scope="custom-fields"
          tableId="custom-fields-table"
          selectedLabel="{{ __('Field(s) selected') }}"
        />
      @endcan
      <div class="card border-0 rounded-2xl shadow-md overflow-hidden my-3">
        <div class="h-1 w-full" style="background:#007C38;"></div>

      <div class="card-body bg-white m-4 table-border-style">
        <div class="table-responsive table-new-design bg-white">
          <table id="custom-fields-table" class="table datatable">
            <thead>
              <tr>
                {{-- master checkbox --}}
                <th data-sortable="false" data-type="html"
                  ...
                  <label class="mcheck">
                    <input type="checkbox" class="jsb-master" data-scope="custom-fields">
                    <span class="box"></span>
                  </label>
                </th>

                <th class="px-4 py-1 border border-[#E5E5E5] bg-[#F6F6F6] font-[600] text-[12px]">{{ __('Custom Field') }}</th>
                <th class="px-4 py-1 border border-[#E5E5E5] bg-[#F6F6F6] font-[600] text-[12px]">{{ __('Type') }}</th>
                <th class="px-4 py-1 border border-[#E5E5E5] bg-[#F6F6F6] font-[600] text-[12px]">{{ __('Module') }}</th>
              ...
              @foreach ($custom_fields as $field)
                <tr>
                  ...
                  <td class="input-checkbox px-4 py-4 text-left text-[12px] border-0 w-12">
                    <label class="mcheck">
                      <input type="checkbox" class="jsb-item" data-scope="custom-fields" value="{{ $field->id }}" data-id="{{ $field->id }}">
                      <span class="box"></span>
                    </label>
                  </td>

                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $field->name }}</td>
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $field->type }}</td>
                  <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $field->module }}</td>
...
@push('script-page')
<script>
  // stop row click when using checkbox/dropdown
