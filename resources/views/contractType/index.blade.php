...
@section('action-btn')
<style>
  /* material checkbox (same used elsewhere) */
...
<div class="flex items-center gap-2 mt-2 sm:mt-0">
    {{-- Export All --}}
    <a href="{{ route('contractType.export') }}"
    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            @can('delete constant contract type')
                <x-bulk-toolbar
                    :deleteRoute="route('contractType.bulk-destroy')"
                    :exportRoute="route('contractType.export-selected')"
                    scope="contract-types"
                    tableId="contract-types-table"
                    selectedLabel="{{ __('Type(s) selected') }}"
                />
            @endcan
            ...
            <div class="card-body bg-white m-4 table-border-style">
                <div class="table-responsive table-new-design">
                    <table id="contract-types-table" class="table datatable">
                        <thead>
                            <tr>
                                {{-- master checkbox --}}
                                <th data-sortable="false" data-type="html"
                                    ...
                                    <label class="mcheck">
                                        <input type="checkbox" class="jsb-master" data-scope="contract-types">
                                        <span class="box"></span>
                                    </label>
                                </th>
                            ...
                            @foreach ($types as $type)
                                <tr class="font-style">
                                    ...
                                    <td class="input-checkbox px-4 py-4 text-left text-[12px] border-0 w-12">
                                        <label class="mcheck">
                                            <input type="checkbox" class="jsb-item" data-scope="contract-types" value="{{ $type->id }}" data-id="{{ $type->id }}">
                                            <span class="box"></span>
                                        </label>
                                    </td>

                                    <td class="px-4 py-3 border border-[#E5E5E5] text-gray-700">{{ $type->name }}</td>

                                    <td class="px-4 py-3 relative border border-[#E5E5E5] text-gray-700 Action">
                                        <button
...
@push('script-page')
<script>
  // prevent row click on checkbox/dropdown
