...
@section('action-btn')
<style>
  /* material checkbox (match customer page) */
...
<script>
  ...
  // top "Export Selected" (same pattern as customers)
  $(document).on('click','[data-export-selected][data-scope="invoices"]',function(e){
    e.preventDefault();
    const scope = $(this).data('scope');       // "invoices"
    const route = $(this).data('route');       // export-selected route
    const key   = 'bulk:'+scope;               // localStorage key maintained by <x-bulk-toolbar/>
    let ids = [];
