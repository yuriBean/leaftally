...
<head>
    ...
    <meta name="description" content="Invoice #{{ Auth::user()->invoiceNumberFormat($invoice->invoice_id) }} from {{ $settings['company_name'] ?? '' }}">
    <meta name="generator" content="LeafTally Invoice System">
    <meta name="created" content="{{ date('Y-m-d H:i:s') }}">
    @php
        $selectedFont = isset($font) ? $font : (isset($settings_data['invoice_font']) ? $settings_data['invoice_font'] : 'Inter');
        $fontUrl = '';
        switch($selectedFont) {
            case 'Inter':
            ...
            case 'Open Sans':
                ...
                break;
            case 'Lato':
                $fontUrl = 'Lato:wght@300;400;700;900';
                break;
            case 'Poppins':
    ...
    @if($fontUrl)
        <link href="https://fonts.googleapis.com/css2?family={{ str_replace(' ', '+', $fontUrl) }}&family=Playfair+Display:wght@400;600;700&display=swap" rel="stylesheet">
    @endif
    ...
    <style type="text/css">
        ...
        body {
            font-family: '{{ $selectedFont }}', -apple-system, BlinkMacSystemFont, sans-serif;
            font-size: 14px;
            line-height: 1.6;
            color: var(--gray-800);
                ...
                <tbody>
                    <tr>
                        <td>
                            ...
                            @else
                                <div class="invoice-logo-placeholder bg-white d-flex align-items-center justify-content-center" style="width: 200px; height: 80px; border: 2px dashed rgba(255,255,255,0.3); border-radius: 8px;">
                            ...
                            <div class="total-section">
                                <table class="total-table">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right amount-highlight">{{ Utility::priceFormat($settings, $invoice->getSubTotal() - $invoice->getTotalDiscount() + $invoice->getTotalTax()) }}</td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('Paid') }}:</td>
                                        <td class="text-right">{{ Utility::priceFormat($settings, $invoice->getTotal() - $invoice->getDue() - $invoice->invoiceTotalCreditNote()) }}</td>
                                    </tr>
                                    @if($invoice->invoiceTotalCreditNote() > 0)
                                        <tr>
                                            <td>{{ __('Credit Note') }}:</td>
                                            <td class="text-right">{{ Utility::priceFormat($settings, $invoice->invoiceTotalCreditNote()) }}</td>
                                        </tr>
                                    @endif
                                    <tr>
                                        <td><strong>{{ __('Amount Due') }}:</strong></td>
                                        <td class="text-right">
                                            <span class="badge {{ $invoice->getDue() > 0 ? 'badge-pending' : 'badge-paid' }}">
