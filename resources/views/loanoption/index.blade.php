...
@section('action-btn')
  @can('create loan option')
    <div class="flex items-center gap-2 mt-2 sm:mt-0 float-end">
      <a href="#"
         data-url="{{ route('loanoption.create') }}"
      ...
      <div class="h-1 w-full" style="background:#007C38;"></div>
        ...
        <div class="table-responsive table-new-design bg-white p-4">
          <table class="table datatable border border-[#E5E5E5] rounded-[8px]">
            <thead>
              <tr>
                <th class="px-4 py-1 border border-[#E5E5E5] bg-[#F6F6F6] font-[600] text-[12px]">{{ __('Loan Option') }}</th>
                      ...
                      @can('delete loan option')
                        <li>
                          ...
                          <a href="#"
                             ...
                             data-confirm-yes="document.getElementById('delete-form-{{ $loanoption->id }}').submit();">
...
@push('script-page')
<script>
  // Match Bills behavior: prevent dropdown/menu clicks from bubbling into datatable
