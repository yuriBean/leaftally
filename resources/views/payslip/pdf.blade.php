                                ...
                                <table class="table table-md">
                                    <tbody>
                                        ...
                                        @foreach ($payslipDetail['earning']['allowance'] as $allowance)
                                            @php $allowance = json_decode($allowance->allowance); @endphp
                                            @foreach ($allowance as $all)
                                                <tr>
                                                    ...
                                                    @if ($all->type != 'percentage')
                                                        <td class="text-end">{{ \Auth::user()->priceFormat($all->amount) }}</td>
                                                    @else
                                                        <td class="text-end">{{ $all->amount }}% ({{ \Auth::user()->priceFormat(($all->amount * $payslip->basic_salary) / 100) }})</td>
                                                    @endif
                                                </tr>
                                            @endforeach
                                        @endforeach
                                        ...
                                        @foreach ($payslipDetail['earning']['commission'] as $commission)
                                            @php $commissions = json_decode($commission->commission); @endphp
                                            @foreach ($commissions as $empcom)
                                                <tr>
                                                    ...
                                                    @if ($empcom->type != 'percentage')
                                                        <td class="text-end">{{ \Auth::user()->priceFormat($empcom->amount) }}</td>
                                                    @else
                                                        <td class="text-end">{{ $empcom->amount }}% ({{ \Auth::user()->priceFormat(($empcom->amount * $payslip->basic_salary) / 100) }})</td>
                                                    @endif
                                                </tr>
                                            @endforeach
                                        @endforeach
                                        ...
                                        @foreach ($payslipDetail['earning']['otherPayment'] as $otherPayment)
                                            @php $otherpay = json_decode($otherPayment->other_payment); @endphp
                                            @foreach ($otherpay as $op)
                                                <tr>
                                                    ...
                                                    @if ($op->type != 'percentage')
                                                        <td class="text-end">{{ \Auth::user()->priceFormat($op->amount) }}</td>
                                                    @else
                                                        <td class="text-end">{{ $op->amount }}% ({{ \Auth::user()->priceFormat(($op->amount * $payslip->basic_salary) / 100) }})</td>
                                                    @endif
                                                </tr>
                                            @endforeach
                                        @endforeach
                                        ...
                                        @foreach ($payslipDetail['earning']['overTime'] as $overTime)
                                            @php $arrayJson = json_decode($overTime->overtime); @endphp
                                            @foreach ($arrayJson as $overtime)
                                                <tr>
                                                    <td>{{ __('OverTime') }}</td>
                                                    <td>{{ $overtime->title }}</td>
                                                    <td>-</td>
                                                    <td class="text-end">{{ \Auth::user()->priceFormat($overtime->number_of_days * $overtime->hours * $overtime->rate) }}</td>
                                                </tr>
                                            @endforeach
                                        @endforeach
                                    </tbody>
                                ...
                                <table class="table table-striped table-hover table-md">
                                    <tbody>
                                        ...
                                        @foreach ($payslipDetail['deduction']['loan'] as $loan)
                                            @php $loans = json_decode($loan->loan); @endphp
                                            @foreach ($loans as $emploanss)
                                                <tr>
                                                    ...
                                                    @if ($emploanss->type != 'percentage')
                                                        <td class="text-end">{{ \Auth::user()->priceFormat($emploanss->amount) }}</td>
                                                    @else
                                                        <td class="text-end">{{ $emploanss->amount }}% ({{ \Auth::user()->priceFormat(($emploanss->amount * $payslip->basic_salary) / 100) }})</td>
                                                    @endif
                                                </tr>
                                            @endforeach
                                        @endforeach
                                        ...
                                        @foreach ($payslipDetail['deduction']['deduction'] as $deduction)
                                            @php $deductions = json_decode($deduction->saturation_deduction); @endphp
                                            @foreach ($deductions as $saturationdeduc)
                                                <tr>
                                                    ...
                                                    @if ($saturationdeduc->type != 'percentage')
                                                        <td class="text-end">{{ \Auth::user()->priceFormat($saturationdeduc->amount) }}</td>
                                                    @else
                                                        <td class="text-end">{{ $saturationdeduc->amount }}% ({{ \Auth::user()->priceFormat(($saturationdeduc->amount * $payslip->basic_salary) / 100) }})</td>
                                                    @endif
                                                </tr>
                                            @endforeach
                                        @endforeach
