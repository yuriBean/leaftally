...
//                        $path = $request->file('document')[$key]->storeAs('uploads/document/', $fileNameToStore);
    ...
    public function exportSelected(Request $request)
    {
        ...
        $ids = $request->input('ids', $request->input('employee_ids', []));
        $validated = validator(['ids' => $ids], [
            'ids'   => ['required','array','min:1'],
            'ids.*' => ['integer'],
        ])->validate();

        $name = 'employee_selected_' . date('Y-m-d_H-i-s') . '.xlsx';
        ...
        public function bulkDestroy(Request $request)
    {
        ...
        return back()->with('success', __(":count employee(s) deleted successfully.", ['count' => $count]));
    }
}
